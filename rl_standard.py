...
RLStandard(Policy) -- fully free action space (raw voltage in
[action_min_v, action_max_v], no phase structure at all) paired with the
"hammer, not magic wand" structured observation (elapsed time, target
power, own last output power, own/adjacent last applied voltage, pulse
phase, in-pulse/post-pulse-off flags -- no v_cap_estimate anywhere).
See the original regular_rl_env.py's module docstring for full rationale.
"""
...
class _ModulatorGymEnv(gym.Env):
    ...
    def __init__(self, rf_amp, dc_bias, v_pi, null_pt, p_in, er, t_res, mod_res, cap_t, cap_p,
        ...
        self.action_space = spaces.Box(
            low=np.full(num_modulators, action_min_v, dtype=np.float32),
            high=np.full(num_modulators, action_max_v, dtype=np.float32),
    ...
    def step(self, action):
        s = self.current_step
        v = np.clip(action, self.action_min_v, self.action_max_v).astype(np.float64)
...
class RLStandard(Policy):
    def __init__(self,
                 conditions: EnvironmentalConditions,
                 action_min_v: float = 0.0,
                 action_max_v: float = 1.5,
    ...
    def train(self, **kwargs) -> None:
        ...
        best_ckpt_dir = tempfile.mkdtemp(prefix="rl_standard_best_")
        try:
    ...
    def save(self, path: str) -> None:
        if self.model is None or self.vec_normalize is None:
            raise RuntimeError("RLStandard.save() called before train(); nothing to save.")
    ...
    def solve(self, active_channels: list[int] | None = None):
        if self.model is None or self.vec_normalize is None:
            raise RuntimeError("RLStandard.solve() called before train()/load(); no model available.")
